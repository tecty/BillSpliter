<template>
  <v-container>
    ...
    <div v-else>
        ...
        <v-flex xs12 md10 lg8 mt-2 mb-2>
          <v-data-table :headers="headers" :items="ongoing" item-key="name" hide-actions>
            <template slot="items" slot-scope="props">
              <router-link tag="tr" :to="{ name: 'billDetail', params: { id: props.item.bill } }">
                {{ props.item.title }}
      ...
      <h2>Consensused Transactions</h2>
      <v-layout row wrap>
        <v-flex xs12 md10 lg8 mt-2 mb-2>
...
export default {
  ...
  methods: {
    ...
    async refreshData() {
      ...
      // these two should be map as computed valu
      this.ongoing = ret[0].data.filter(
        el => el.from_u == this.uid && el.state == "PR"
      );
      this.concencus = ret[0].data.filter(
